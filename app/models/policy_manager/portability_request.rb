...
module PolicyManager
  class PortabilityRequest < ApplicationRecord
    ...
    def notify_progress
      PortabilityMailer.progress_notification(self.id).deliver!
    end

    def notify_progress_to_admin
      PortabilityMailer.admin_notification(self.user.id).deliver!
    end

    def notify_completeness
      PortabilityMailer.completed_notification(self.id).deliver!
    end
